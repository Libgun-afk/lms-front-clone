...
function BranchList() {
  ...
  const filteredBranches = branches.filter((branch: { status: string; name: string; }) => {
    const statusMatch = selectedStatus.length === 0 || selectedStatus.includes(branch.status);
  ...
  const columns: TableProps<typeof branches[0]>["columns"] = [
    ...
    },
    {
      ...
      dataIndex: "imageUrl",
      key: "imageUrl",
      render: (imageUrl) => <Image src={imageUrl} alt="Branch Image" width={50} height={50} />,
    },
    {
    ...
    <div className="flex w-full gap-3" onSubmit={handleSubmit}>
      ...
      <div className="w-72 p-4 border-r bg-gray-50 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Checkbox.Group
            className="flex flex-col gap-2"
            options={["Хангамж нийлүүлэгч", "Нийлүүлэгч", "Гүйцэтгэгч компани", "Логистик болон тээвэрлэгч"]}
      ...
      <div className="flex flex-col gap-4 w-full rounded-xl bg-white  dark:bg-gray-700 p-4">
        <div className="flex justify-between  h-[60px] items-center border-b border-gray-200 px-4 ">
          ...
          <div className="relative grid grid-cols-2 bg-gray-100 dark:bg-gray-800 w-[400px] h-9 rounded-xl border border-gray-300 dark:border-gray-700 overflow-hidden">
            <div
              className={"absolute top-[2px] bottom-[2px] w-[calc(49%-3px)] bg-white dark:bg-gray-700 rounded-xl transition-transform duration-300 " +
                (activeTab === "supply"
                  ? "translate-x-[calc(100%+12px)]"
                  : "translate-x-[2px]")} />
            <button
              onClick={() => setActiveTab("branch")}
              className={`relative z-30 px-2 py-1.5  w-[200px] text-center flex justify-center pt-1 font-medium transition-all duration-300 rounded-xl ${activeTab === "branch"
                  ? "text-black"
            ...
            <button
              ...
              className={`relative z-10 px-2 py-1.5 w-[200px] text-center flex justify-center pt-1 font-medium transition-all duration-300 rounded-xl ${activeTab === "supply"
                  ? "text-black"
                  : "text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-300"}`}
            >
              Газрын зургаар
